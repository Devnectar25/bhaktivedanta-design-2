            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 pt-6">
                {(aboutState.newDevelopments || []).map((dev, idx) => {
                  const isFeatured1 = idx === 0;
                  const isFeatured2 = idx === 1;
                  const isSidebar = idx >= 2 && idx <= 5;
                        ...
                        <div className={`px-4 py-2 text-xs font-bold flex items-center justify-between border-b ${
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <span>
                              ...
                              {isSidebar && `📌 Home Sidebar #${idx - 1}`}
                              {!isFeatured1 && !isFeatured2 && !isSidebar && `Post #${idx + 1} (Archives)`}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              type="button"
                              disabled={idx === 0}
                              onClick={() => handleMoveDev(idx, -1)}
                              title="Move Up"
                              className={`p-1 rounded transition-colors ${idx === 0 ? 'opacity-30 cursor-not-allowed' : 'bg-black/10 hover:bg-black/20 text-current'}`}
                            >
                              <ArrowUp size={12} />
                            </button>
                            <button
                              type="button"
                              disabled={idx === (aboutState.newDevelopments?.length || 1) - 1}
                              onClick={() => handleMoveDev(idx, 1)}
                              title="Move Down"
                              className={`p-1 rounded transition-colors ${idx === (aboutState.newDevelopments?.length || 1) - 1 ? 'opacity-30 cursor-not-allowed' : 'bg-black/10 hover:bg-black/20 text-current'}`}
                            >
                              <ArrowDown size={12} />
                            </button>
                          </div>
