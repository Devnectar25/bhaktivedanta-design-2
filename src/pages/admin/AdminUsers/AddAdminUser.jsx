...
const AddAdminUser = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!username || !email) {
      showErrorAlert("Missing Information", "Please fill in Username and Email.");
      return;
    }
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <nav className="flex items-center gap-1.5 text-slate-400 text-xs mb-2 font-medium font-sans">
        ...
        <span className="material-symbols-outlined text-xs">chevron_right</span>
        <span className="text-slate-600 font-bold">Add Admin User</span>
      </nav>

      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Add New Admin User</h2>
        <p className="text-sm text-slate-500 font-medium font-sans">Configure access details and role authorizations for an administrator account.</p>
      </div>
